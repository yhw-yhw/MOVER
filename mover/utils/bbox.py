...
from mover.utils.detectron2_boxmode import BoxMode
import cv2
...
def write_opencv_matrix(fn, cam_inc, cam_ext):
    cv_file = cv2.FileStorage(fn, cv2.FILE_STORAGE_WRITE)
    cam_ext = np.concatenate((cam_ext, np.zeros((3, 1))), axis=1)
    cv_file.write("CameraMatrix", cam_ext)
    cv_file.write("Intrinsics", cam_inc)
    cv_file.write("Distortion", np.zeros((8, 1)))
