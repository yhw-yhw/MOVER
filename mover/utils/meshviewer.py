...
class Mesh(trimesh.Trimesh):
    ...
    def set_vertex_colors(self,vc, vertex_ids = None): #1x3
...
class MeshViewer(object):
    ...
    def __init__(self,
        ...
        assert cam_inc is not None
        assert cam_ext is not None
        fx, fy = cam_inc[0][0], cam_inc[1][1]
        cx, cy = cam_inc[0][2], cam_inc[1][2] 
        print(fx, fy, cx, cy)
        # import pdb;pdb.set_trace
        pc = pyrender.camera.IntrinsicsCamera(fx=fx, fy=fy,
            cx=cy, cy=cy)
        if cam_ext.shape[-1] == 3:
            camera_pose = np.concatenate((cam_ext, np.zeros((3, 1))), 1)
            camera_pose = np.concatenate((camera_pose, np.array([[0, 0, 0, 1]])), 0)
        else:
            camera_pose = cam_ext
    ...
    def render(self, flags=pyrender.RenderFlags.RGBA):
        color, _ = self.viewer.render(self.scene, flags=pyrender.RenderFlags.RGBA)
        color = color.astype(np.float32) / 255.0

        valid_mask = (color[:, :, -1] > 0)[:, :, np.newaxis]
        return color[:, :, :-1], valid_mask
