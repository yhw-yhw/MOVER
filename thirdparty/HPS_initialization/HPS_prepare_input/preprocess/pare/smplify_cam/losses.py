...
from ..utils.geometry import perspective_projection, weak_perspective_projection
...
def body_fitting_loss(body_pose, betas, model_joints, camera_t, camera_center,
                      joints_2d, joints_conf, pose_prior, camera_rotation,
                      focal_length=5000, sigma=100, pose_prior_weight=4.78,
                      shape_prior_weight=5, angle_prior_weight=15.2,
                      output='sum', use_weak_perspective=False, img_res=224):
    """
    ...
    if use_weak_perspective:
        projected_joints = weak_perspective_projection(model_joints, camera_rotation,
                                                       camera_t, focal_length, camera_center, img_res)
    else:
        projected_joints = perspective_projection(model_joints, camera_rotation, camera_t, focal_length, camera_center)
...
def camera_fitting_loss(model_joints, camera_t, camera_t_est, camera_center, joints_2d, joints_conf,
                        focal_length, camera_rotation, depth_loss_weight=100, use_weak_perspective=False,
                        img_res=224, use_all_joints_for_camera=False):
    """
    ...
    if use_all_joints_for_camera:
        is_valid = (joints_conf > 0.5).float()
        reprojection_loss = (is_valid * (joints_conf ** 2) * reprojection_error.sum(dim=-1)).sum()
    else:
