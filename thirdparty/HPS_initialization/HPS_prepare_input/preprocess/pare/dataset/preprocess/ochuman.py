...
def get_eft_fitter(split_idx=-1):
    ...
    hparams.LOG = True
    hparams.LOG_DIR = 'logs/eft/ochuman'
...
def ochuman_extract(dataset_path, out_path, split_idx=-1, num_splits=1):
    ...
    if num_splits == 1:
        ...
        json_data = np.array_split(json_data['images'], num_splits)[split_idx]
    ...
    for img_data in tqdm(json_data):
        ...
        for annot in img_data['annotations']:
            ...
            # keypoints
            part = convert_kps(keypoints[None,...], src='ochuman', dst='spin')[0]
    ...
    if num_splits == 1:
        ...
        out_file = os.path.join(out_path, 'ochuman_train.npz')
    else:
        # store the data struct
        out_path = out_path + '/ochuman'
        if not os.path.isdir(out_path):
            os.makedirs(out_path)
        out_file = os.path.join(out_path, f'ochuman_train_{split_idx}-{num_splits}.npz')
    np.savez(
        out_file,
        imgname=imgnames_,
        center=centers_,
