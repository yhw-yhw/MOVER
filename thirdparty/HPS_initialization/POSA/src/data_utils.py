...
from src import eulerangles, misc_utils, posa_utils
import pickle
import json
...
# TODO: 
# the original CS is in camera CS.
# transform into canonical CS.
def pkl_to_canonical(pkl_file_path, device, dtype, batch_size, gender='male', model_folder=None, vertices_clothed=None,
                     **kwargs):
    # R_can = torch.tensor(eulerangles.euler2mat(np.pi, np.pi, 0, 'syzx'), dtype=dtype, device=device)
    
    R_smpl2scene = torch.tensor(eulerangles.euler2mat(np.pi / 2, 0, 0, 'sxyz'), dtype=dtype, device=device) # change
    # import pdb;pdb.set_trace()
    ...
    vertices = body_model_output.vertices.squeeze()
    # R_can_t1 = torch.tensor(eulerangles.euler2mat(np.pi, np.pi, 0, 'syzx'), dtype=dtype, device=device)
    R_can_t1 = torch.tensor(eulerangles.euler2mat(0, 0, 0, 'syzx'), dtype=dtype, device=device)
    vertices_tmp = torch.matmul(R_can_t1, (vertices - pelvis).t()).t()

    # orientaion around the y-axis
    # R_can_t1 = torch.tensor(eulerangles.euler2mat(np.pi, 0, 0, 'syzx'), dtype=dtype, device=device)
    # vertices_tmp = torch.matmul(R_can_t1, vertices_tmp.t()).t()

    # import pdb;pdb.set_trace()
    R_can = compute_canonical_transform(body_model.global_orient)

    vertices_can = torch.matmul(R_can, vertices_tmp.t()).t()
    vertices = torch.matmul(R_smpl2scene, vertices_tmp.t()).t()
...
def pkl_to_canonical_agora(pkl_file_path, device, dtype, batch_size, gender='male', model_folder=None, vertices_clothed=None,
                     **kwargs):
    R_can = torch.tensor(eulerangles.euler2mat(np.pi, np.pi, 0, 'syzx'), dtype=dtype, device=device)
    R_smpl2scene = torch.tensor(eulerangles.euler2mat(np.pi / 2, 0, 0, 'sxyz'), dtype=dtype, device=device)
    with open(pkl_file_path, 'rb') as f:
...
# virtualhome CS -> posa CS
# transform into canonical CS.
def pkl_to_canonical_vh2posa(pkl_file_path, device, dtype, batch_size, gender='male', model_folder=None, vertices_clothed=None,
                     **kwargs):
    # R_can = torch.tensor(eulerangles.euler2mat(np.pi, np.pi, 0, 'syzx'), dtype=dtype, device=device)
    ...
    # orientaion around the y-axis, clock-wise
    # R_can_t1 = torch.tensor(eulerangles.euler2mat(np.pi, 0, 0, 'syzx'), dtype=dtype, device=device)
    # vertices_tmp = torch.matmul(R_can_t1, vertices_tmp.t()).t()
    ...
    R_cs = torch.tensor([[0, 0, 1],[0, -1, 0], [1, 0, 0]], dtype=dtype, device=device)
    R_cs_tmp = torch.tensor(eulerangles.euler2mat(np.pi/2, 0, 0, 'syxz'), dtype=dtype, device=device)
    # import pdb;pdb.set_trace()
    new_vertices_can = torch.matmul(R_cs, vertices_can.t()).t()
    new_vertices_can = torch.matmul(R_cs_tmp, new_vertices_can.t()).t()
    # vertices_can = torch.matmul(R_can, (vertices - pelvis).t()).t()
    # vertices = torch.matmul(R_smpl2scene, (vertices - pelvis).t()).t()

    return vertices, new_vertices_can, faces_arr, body_model, R_can, pelvis, torch_param, vertices_clothed
