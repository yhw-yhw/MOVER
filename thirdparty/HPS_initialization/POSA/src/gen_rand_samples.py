...
if __name__ == '__main__':
    ...
    for pkl_file_path in pkl_file_paths: #[:1]: # debug
        print('file_name: {}'.format(pkl_file_path))
        pkl_file_basename = osp.splitext(osp.basename(pkl_file_path))[0]

        # import pdb;pdb.set_trace()
        # load pkl file
        if cs_name == 'virtualhome':
            print(f'run virtualhome')
            vertices, vertices_can, faces_arr, body_model, R_can, pelvis, torch_param, _ = data_utils.pkl_to_canonical_vh2posa(
                pkl_file_path, device, dtype, **args_dict)
        elif cs_name == 'agora':
            vertices, vertices_can, faces_arr, body_model, R_can, pelvis, torch_param, _ = data_utils.pkl_to_canonical_agora(
                pkl_file_path, device, dtype, **args_dict)
        else: # default: opencv camera CS.
        ...
        if args.render:
            ...
            if True:
                # gen_batch = gen_batch.detach().cpu().numpy()
                img = viz_utils.render_sample(gen_batch, vertices_can, faces_arr, **args_dict)
                for i in range(args.num_rand_samples):
                    img[i].save(osp.join(rand_samples_dir, '{}_sample_{:02d}_can.png'.format(pkl_file_basename, i)))

        if args.save:
            if type(gen_batch) is torch.Tensor:
                gen_batch = gen_batch.detach().cpu().numpy()
            save_dir = args.save_dir
            os.makedirs(save_dir, exist_ok=True)
            for i in range(args.num_rand_samples):
                save_fn = osp.join(save_dir, '{}_sample_{:02d}.npy'.format(pkl_file_basename, i))
                np.save(save_fn, gen_batch[i])
