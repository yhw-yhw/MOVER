...
class PointsField(Field):
    ...
    def load(self, model_path, idx, category):
        ...
        file_path = os.path.join(model_path + '.obj')
        mesh = trimesh.load(file_path, process=False)
        # import pdb;pdb.set_trace()
        points = mesh.vertices * 0.5
        # old_points = points - points.mean(0)
        old_points = points
        # change pose
        points = np.stack([old_points[:, 2], old_points[:, 1], old_points[:, 0]], axis=1)
        ...
        if points.dtype == np.float16:
            ...
            points = points.astype(np.float32)
        npz_formate_data = {None: np.array(points), 'scale': 1.0}
        
        if self.transform is not None:
            data = self.transform(npz_formate_data)
        # import pdb;pdb.set_trace()
        ...
        # ])
        # # import pdb;pdb.set_trace()
        # points = self.transform(points[None]).squeeze()
        # data = {
        #     None: points,
        #     # 'occ': occupancies,
        # }
